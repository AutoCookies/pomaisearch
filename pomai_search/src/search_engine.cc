 ...
 public:
  Shard(int dim, size_t alignment, size_t reserve_vectors, const SearchEngineConfig& cfg)
    ...
    if (cfg.index_type == SearchEngineConfig::IndexType::Hnsw) {
      uint32_t seed = static_cast<uint32_t>(cfg.global_seed ^ StableHash64("hnsw_seed"));
...
StatusOr<SearchResponse> SearchEngine::SearchWithExplain(VectorView q, QueryOptions opt,
  ...
  if (impl_->query_pool && opt.scope == QueryOptions::Scope::Global) {
    ...
    size_t limit = opt.scope == QueryOptions::Scope::Local ? 1 : impl_->shards.size();
