...
POMAI_TEST(VectorStoreAppend) {
  ...
  size_t off1 = store.Insert(a, guard);
  size_t off2 = store.Insert(b, guard);
...
POMAI_TEST(VectorStoreAlignment) {
  ...
  auto guard = store.AcquireWrite();
  size_t offset = store.Insert(v, guard);
  auto read_guard = store.AcquireRead();
...
POMAI_TEST(VectorStoreReuseSlots) {
  ...
  float c[2] = {5.0f, 6.0f};
  auto guard = store.AcquireWrite();
  size_t slot1 = store.Insert(a, guard);
  size_t slot2 = store.Insert(b, guard);
  store.Release(slot1, guard);
  size_t slot3 = store.Insert(c, guard);
